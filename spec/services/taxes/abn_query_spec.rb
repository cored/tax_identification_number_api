...
RSpec.describe Taxes::AbnQuery do
	let(:abn) { Taxes::Countries::ABN.new(tin: "51824753556") }
	...
	describe ".call" do
		...
		context "when ABN is not registered for GST", :vcr do
			let(:abn) { Taxes::Countries::ABN.new(tin: "51824753557") }

			it "raises an InvalidTIN error" do
				expect { abn_query.call(abn) }.to raise_error(Taxes::InvalidTIN, "ABN is not registered for GST")
			end
		end
	end
end
