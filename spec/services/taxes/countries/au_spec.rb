...
RSpec.describe Taxes::Countries::AU do
  subject(:tin_au) { described_class.from(tin, abn_query: abn_query) }
  let(:abn_query) { class_double(Taxes::AbnQuery, call: abn_query_response) }
  let(:abn_query_response) do
    Taxes::AbnQuery::Response.new(
      abn: '51824753556',
      status: 'Active',
      entity_type: 'Company',
  ...
  describe '.from' do
    ...
    context 'when the TIN is invalid' do
      ...
      it 'raises an error message' do
        expect { tin_au }.to raise_error(Taxes::InvalidTIN)
      end
    end
  end
end
